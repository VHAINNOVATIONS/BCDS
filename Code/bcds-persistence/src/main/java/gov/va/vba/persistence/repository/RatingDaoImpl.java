...
public class RatingDaoImpl implements RatingDao {
    ...
    public List<Long> getKneeCntntPattrens(Map<Long, Integer> contentionCount, List<Long> pattrens, String modelType) {
        ...
        pattrenQuery.append("SELECT DISTINCT PATTERN_ID FROM BCDSS.DDM_MODEL_CNTNT WHERE UPPER(MODEL_TYPE)='" + modelType + "' AND (  ");
    ...
    public List<Long> getKneeDiagPattrens(List<DiagnosisCount> diagCount, List<Long> pattrens, String modelType) {
        StringBuilder pattrenQuery = new StringBuilder();
        pattrenQuery.append("SELECT DISTINCT PATTERN_ID FROM BCDSS.DDM_MODEL_DIAG WHERE UPPER(MODEL_TYPE)='" + modelType + "' AND (  ");
    ...
    public ContentionDetails getContention(long contentionCode) {
        ...
        LOG.info("****************************************************************");
        LOG.info("Contention Details" + contentionDetails.toString());
        LOG.info("****************************************************************");
    ...
    public Date getBeginDate(long veteranId, Date claimDate) {
        Date beginDate = jdbcTemplate.queryForObject(QueryConstants.KNEE_PRIOR_CDD_BEGIN_DATE, new Object[]{claimDate, veteranId, claimDate, veteranId}, Date.class);
        LOG.info("****************************************************************");
