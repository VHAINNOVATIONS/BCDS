...
$.widget(widgetName, {
	...
	_initEvents: function() { //initialize all event listeners
		...
		this.input.bind("keydown keypress keyup", function(event) {
			...
			case keyCodes.DOWN:
				// VBMS-R change: Modified so Down Arrow selects next item; Alt + Down Arrow opens dropdown
				if (event.altKey) {
					self.showList();
				}
				else {
			...
			default:
				// VBMS-R change: Modified to support showAndFilterOnKeyUp option
		...
		this.input.bind("click", function(e) {
			...
			if (!self.listVisible()) { 
				...
				// VBMS-R change: Modified to add parameter to not select all text
				self.inputFocus(true);
				// VBMS-R change: End Modified
				
				// VBMS-R change: Removed to support showListOnFocus option
				//self.showList();
				// VBMS-R change: End Removed
				
				// VBMS-R change: Added to support showListOnFocus option
				if (self.internalFocus && self.options.showListOnFocus) {
					self.showList();
				}
				// VBMS-R change: End Added
...
$.extend($.ui.ufd, {
	...
	defaults: { // 1.7 default options location, see below
        ...
        // VBMS-R change: Added showListOnFocus and showAndFilterOnKeyUp options
