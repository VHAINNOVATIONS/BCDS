...
angular.module('bcdssApp').controller('DDMController', function($rootScope, $scope, $state, Account, $stateParams, DDMService) {
    $scope.searchTerm = undefined;
    $scope.DDM = [];
    $scope.orderByField = 'patternId';
    ...
    $scope.toggleCheckAll = function () {
        angular.forEach($scope.DDM, function (DDM) {
            DDM.Selected = $scope.selectAll;
        });
    };  
    
    $scope.loadDDM = function(){
    	DDMService.query(function(result){
    		$scope.DDM = result;
    	});
    };
...
/*        $scope.search = function(searchTerm) {
        ...
        $scope.deleteDDM = function(DDM) {
            var index = $scope.DDM.indexOf(DDM);
            $scope.DDM.splice(index,1);
        }
