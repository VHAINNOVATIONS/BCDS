...
SQL="select rd.ptcpnt_vet_id, rd.prfil_dt, rd.begin_dt, rd.end_dt, rd.prmlgn_dt, rd.dgnstc_txt, rd.dsblty_id, rd.diagnosis_code, rd.hypntd_dgnstc_type_cd, rd.prcnt_nbr \
	...
	inner join AGGREGATE_CONTENTION ac on ac.vet_id = rd.ptcpnt_vet_id \
	...
	and ac.vet_id < 11088698 \ #This is only utilized to govern the next set of records to include, occurs when you build up records in chunks rather than one full run.
...
writeCursor.prepare('INSERT INTO DEVELOPER.AGGREGATE_DECISION (VET_ID, PROFILE_DATE, PROMULGATION_DATE, RECENT_EAR_DATE, CDD, EAR_CDD, A6100, A6200,A6201,A6202,A6204,A6205,A6207,A6209,A6210,A6211,A6260, TXT_BILATER,TXT_LEFT,TXT_RIGHT,TXT_HEARING,TXT_LOSS,TXT_TINITU,TXT_CONDITION) \
...
:TXT_BILATER, :TXT_LEFT, :TXT_RIGHT, :TXT_HEARING, :TXT_LOSS, :TXT_TINITU, :TXT_CONDITION)')
...
for row in cursor:
	...
	if currRatingProfile != decision.prfil_dt: #Process insert statement and reset aggregation variables when profile date changes
		if currRatingProfile != -1:	#Skip if first time through
			...
			'TXT_BILATER' :aggregateDecision.TXT_BILATER, 'TXT_LEFT' :aggregateDecision.TXT_LEFT, 'TXT_RIGHT' :aggregateDecision.TXT_RIGHT, 'TXT_HEARING' :aggregateDecision.TXT_HEARING, 'TXT_LOSS' :aggregateDecision.TXT_LOSS, 'TXT_TINITU' :aggregateDecision.TXT_TINITU, 'TXT_CONDITION' :aggregateDecision.TXT_CONDITION})
	...
	try:
		if re.search("Bilater",decision.dgnstc_txt,re.IGNORECASE):
		...
		if re.search("Hearing",decision.dgnstc_txt,re.IGNORECASE):
			aggregateDecision.TXT_HEARING = 1
		...
		if re.search("6100|6200|6201|6202|6204|6205|6207|6209|6210|6211|6260",decision.dgnstc_txt,re.IGNORECASE):
			aggregateDecision.TXT_CONDITION = 1
	except TypeError:
		print(decision)
