...
SQL="select rcc.ptcpnt_vet_id, \
  ...
  inner join claim_source cs on cs.vet_id = rcc.ptcpnt_vet_id and cs.claim_id = rcc.bnft_claim_id \
	where prfil_dt >= date_of_claim \
	and cs.vet_id < 11088698 \ #This is only utilized to govern the next set of records to include, occurs when you build up records in chunks rather than one full run.
...
writeCursor.prepare('INSERT INTO DEVELOPER.AGGREGATE_CONTENTION (VET_ID, CLAIM_ID, END_PRODUCT_CODE, CLAIM_DATE, CONTENTION_COUNT, EAR_CONTENTION_COUNT, C2200,C2210, C2220,C3140,C3150,C4130,C4210,C4700,C4920,C6850,TXT_BILATER,TXT_LEFT,TXT_RIGHT,TXT_HEARING,TXT_LOSS,TXT_TINITU, DOB, RO_NUMBER, MAX_PROFILE_DATE) \
VALUES (:VET_ID, :CLAIM_ID, :END_PRODUCT_CODE, :CLAIM_DATE, :CONTENTION_COUNT, :EAR_CONTENTION_COUNT, \
 ...
 :TXT_BILATER, :TXT_LEFT, :TXT_RIGHT, :TXT_HEARING, :TXT_LOSS, :TXT_TINITU, \
...
for row in cursor:
	...
	if currBenefitClaim != contention.bnft_claim_id: #Process insert statement and reset aggregation variables when claim id changes
		...
		if currBenefitClaim != -1: #Skip if first time through
			...
			'C2200' :aggregateContention.C2200, 'C2210' :aggregateContention.C2210, 'C2220' :aggregateContention.C2220, 'C3140' :aggregateContention.C3140, 'C3150' :aggregateContention.C3150, 'C4130' :aggregateContention.C4130, 'C4210' :aggregateContention.C4210, 'C4700' :aggregateContention.C4700, 'C4920' :aggregateContention.C4920, 'C6850' :aggregateContention.C6850,
			'TXT_BILATER' :aggregateContention.TXT_BILATER, 'TXT_LEFT' :aggregateContention.TXT_LEFT, 'TXT_HEARING' :aggregateContention.TXT_HEARING,	'TXT_LOSS' :aggregateContention.TXT_LOSS, 'TXT_RIGHT' :aggregateContention.TXT_RIGHT, 'TXT_TINITU' :aggregateContention.TXT_TINITU,
	...
	if re.search("Bilater",contention.cntntn_clmant_txt,re.IGNORECASE):
		aggregateContention.TXT_BILATER = 1
	if re.search("Left",contention.cntntn_clmant_txt,re.IGNORECASE):
		aggregateContention.TXT_LEFT = 1
	if re.search("Hearing",contention.cntntn_clmant_txt,re.IGNORECASE):
		aggregateContention.TXT_HEARING = 1
	if re.search("Loss",contention.cntntn_clmant_txt,re.IGNORECASE):
		aggregateContention.TXT_LOSS = 1
